...
static int pathparser_path_valid_format(const char *filename){
    int len=strnlen(filename,AeOS_MAX_PATH);        // we'll use digits for our drive numbers
...
static int pathparser_get_drive_by_path(const char **path){
    ...
    // add 3(bytes[size of unsigned char]) to skip ':/'
    *path+=3;
    return drive_num;
}
...
static const char *pathparser_get_path_part(const char **path){
    char *result_path_part= kzalloc(AeOS_MAX_PATH);     // safely use the path variable
    int i=0;
...
struct path_part *pathparser_parse_path_part(struct path_part *last_path,const char ** path){
